...
router.delete('/suggest/PostView/:no/process/delete', (req, res) => {
  ...
  pool.query('SELECT user_id FROM suggests WHERE no = ?', [postId], (error, results) => {
    if (error) {
...
// 게시글 수정
router.post('/suggest/PostView/:no/process/update', upload.single('file'), (req, res) => {
  if (!isLoggedIn(req)) {
...
router.delete('/suggest/PostView/:no/process/deleteFile', (req, res) => {
  ...
  pool.query('SELECT user_id FROM suggests WHERE no = ?', [postId], (error, results) => {
    ...
    pool.query(`UPDATE suggests SET file_data = NULL WHERE no = ?`, [postId], (error) => {
      if (error) {
