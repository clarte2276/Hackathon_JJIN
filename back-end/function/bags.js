...
router.post('/bags/form', (req, res) => {
  ...
  pool.query(`SELECT NOW()`, (err, currentHourRows) => {
    ...
    `, [bag_id, reservation_hour, currentDateString], (err, existingReservations) => {
      if (err) {
      ...
      `, [userId, reservation_hour, currentDateString], (err, userSameTimeReservations) => {
        if (err) {
        ...
        if (userSameTimeReservations[0].count > 0) {
          return res.status(400).json({ message: '동일 시간대에 이미 예약한 좌석이 있습니다.' });
        }
        ...
        `, [userId, currentDateString], (err, userReservations) => {
          ...
          const reservationCount = userReservations.length > 0 ? userReservations[0].reservation_count : 0;

          if (reservationCount >= 2) {
            return res.status(400).json({ message: '하루 최대 예약 가능 시간은 2시간입니다.' });
          }
          ...
          pool.query(`
            ...
            VALUES (?, ?, ?, ?)
          `, [userId, bag_id, reservation_hour, currentDateString], (err) => {
            if (err) {
