...
// 관리자 확인 함수
const isAdmin = (req) => {
  return req.session.user && req.session.user.name === 'Admin' && req.session.user.id === 1111;
};
...
router.get('/notice', (req, res) => {
  ...
  pool.query(
    `SELECT no, user_id, title, content, DATE_FORMAT(created_date, '%y.%m.%d') AS created_date, file_data FROM notice ORDER BY created_date DESC`,
    (error, results) => {
      if (error) {
        console.error('데이터베이스 오류:', error);
        res.status(500).json({ error: '서버에서 게시판 데이터를 불러오는 중 오류가 발생했습니다.' });
      } else {
        res.json({ admin: adminStatus, user: req.session.user, posts: results });
      }
    }
  );
});
...
router.get('/notice/image/:id', (req, res) => {
  ...
  pool.query(`SELECT file_data FROM notice WHERE no = ?`, [id], (error, results) => {
    if (error) {
